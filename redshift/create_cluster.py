...
def create_iam_role(iam):
    ...
    try:
        ...
        iam.attach_role_policy(RoleName=DWH_IAM_ROLE_NAME, PolicyArn=S3_READ_ARN)
...
def main(args):
    ...
    if args.delete:
        ...
        for _ in range(int(600 / timestep)):
            cluster = redshift.describe_clusters(ClusterIdentifier=DWH_CLUSTER_ID)[
                "Clusters"
            ][0]
            if cluster["ClusterStatus"] == "available":
                break
            logging.info(
                'Cluster status is "{}". Retrying in {} seconds.'.format(
                    cluster["ClusterStatus"], timestep
                )
            )
            time.sleep(timestep)
